...
import { academicHistoryAPI } from '../api/academicHistoryAPI.js';
...
function setupFormHandler() 
{
    ...
    form.addEventListener('submit', async e => 
    {
        ...
        try 
        {
            if (relation.id) 
            {
                await academicHistoryAPI.update(relation);
            } 
            else 
            {
                await academicHistoryAPI.create(relation);
            }
            clearForm();
            loadRelations();
        } 
...
async function loadRelations() 
{
    try 
    {
        const relations = await academicHistoryAPI.fetchAll();
...
function renderRelationsTable(relations) 
{
    ...
    relations.forEach(rel => 
    {
        ...
        tr.appendChild(createCell(rel.student_name));
...
async function confirmDelete(id) 
{
    ...
    try 
    {
        await academicHistoryAPI.remove(id);
        loadRelations();
    } 
    catch (err) 
    {
